...
import { ScrollView, Text, View, ActivityIndicator, Pressable, Dimensions } from 'react-native';
  ...
  return (
    <View style={{ height }}>
      ...
      <View className="flex-row justify-between mt-1">
        {data.map((item, index) => (
          <Text key={index} className="flex-1 text-center text-xs text-gray-500" numberOfLines={1}>
            {item.label}
          </Text>
        ))}
      </View>
    ...
    <View style={{ height }}>
      <View className="flex-row items-end justify-between" style={{ height: barAreaHeight }}>
        {data.map((item, index) => {
          ...
          return (
            <View key={index} className="flex-1 items-center justify-end px-0.5">
              {total > 0 && <Text className="text-xs text-gray-600 mb-0.5">{total}</Text>}
...
function RevenueChartsSection({ period }: { period: Period }) {
  ...
  const totalForBreakdown = data.totalServiceRevenueDollars + data.totalProductRevenueDollars || 1;
    ...
    <View className="space-y-4">
      ...
      <View className="flex-row gap-3">
        <Card className="flex-1 p-4">
          ...
          <Text className="text-2xl font-bold text-gray-900">+{data.newUsersInPeriod}</Text>
        </Card>
        <Card className="flex-1 p-4">
          <Text className="text-xs text-gray-500 mb-1">New Clients</Text>
          <Text className="text-2xl font-bold text-blue-600">+{data.newClientsInPeriod}</Text>
        </Card>
        <Card className="flex-1 p-4">
          <Text className="text-xs text-gray-500 mb-1">New Barbers</Text>
          <Text className="text-2xl font-bold text-purple-600">+{data.newBarbersInPeriod}</Text>
        </Card>
      </View>
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16 }}>
        ...
        {activeTab !== 'services' && <PeriodSelector value={period} onChange={setPeriod} />}
