...
const SERVICE_CATEGORIES = ['Haircut', 'Beard & Shave', 'Color', 'Treatment', 'Kids', 'Other'];
      ...
      >
        <View className="bg-white rounded-t-3xl max-h-[90%]">
          ...
          <View className="px-6 pb-8 pt-4 border-t border-gray-100">
            <Button onPress={handleSave}>{initial?.id ? 'Save Changes' : 'Add Service'}</Button>
          </View>
        </View>
      </KeyboardAvoidingView>
    </Modal>
...
export default function ServicesScreen() {
  ...
  const handleDeleteService = (serviceId: string, serviceName: string) => {
    Alert.alert('Delete Service', `Remove "${serviceName}" from your services?`, [
